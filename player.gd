...
@onready var Andar: AudioStreamPlayer2D = $Andar# Nó de áudio para andar
...
var pode_tocar_pulo = true

func _ready():
	$AnimatedSprite2D.scale.x = -abs($AnimatedSprite2D.scale.x)
	$ColorRect.position.x = -65
	print(Andar)
	print("AndarPlayer: ", $AndarPlayer)
	print("PuloPlayer: ", $PuloPlayer)
	print("ActionableFinder: ", $Direction/ActionableFinder)
...
func _movimentar_personagem(delta):
	...
	if  pode_tocar_pulo:
		Pulo.play()
		pode_tocar_pulo = false
		
		
	if is_on_floor():
		pode_tocar_pulo = true
